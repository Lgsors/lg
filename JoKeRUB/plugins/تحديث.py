...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ᯽︙ تم تحديث سورس LG بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ يـتـم البـحـث عـن تـحديثـات سـورس LG انـتـظـر**")
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس LG ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس LG انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ جارِ تحديث ريبو التنصيب لسورس LG **")
    off_repo = "https://github.com/Lgsors/test"
    os.chdir("/app")
    try:
        txt = (
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("**᯽︙ جارِ اعادة تنصيب سورس LG, انتظر قليلاً ..**")
    await deploy(event, repo, ups_rem, ac_br, txt)


progs = [6801329147, 6801329147, 6801329147,6801329147]
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "الان":
            await event.edit("** ᯽︙ يتم تحديث سورس LG بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
